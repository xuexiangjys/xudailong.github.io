---
layout: post
title:  "RxBus  一个简易、非反射的Android事件通知库"
date:   2018-04-17 01:11:24
categories: Android框架
tags: Android RxJava Javassist 
author: xuexiangjys
---
...
![rxbus](https://img-blog.csdn.net/20180418005307680?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3h1ZXhpYW5nanlz/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
...
方法一：使用`RxBusUtils.get().onMainThread`方法注册事件，并指定订阅发生在主线程。
...
方法二：使用`RxBusUtils.get().on`方法注册事件，订阅所在线程为事件发生线程，也可指定订阅发生的线程。
...
方法一：使用`RxBusUtils.get().post(Object eventName)`发送不带数据的事件。

```
RxBusUtils.get().post(EventKey.EVENT_NO_DATA);
```

方法二：使用`RxBusUtils.get().post(Object eventName, Object content)`发送携带数据的事件。
...
方法三：使用`@Bus`进行事件注册和注销的需使用RxEventUtils进行事件方式。
...
3.使用`@Bus`注册的事件，如果是在Activity或者Fragment中，框架会自动进行事件注销。
...
![](https://img-blog.csdn.net/20180418005331186?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3h1ZXhpYW5nanlz/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
